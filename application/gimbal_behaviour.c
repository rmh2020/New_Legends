...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    else if(gimbal_control_set->gimbal_control_way == AUTO)  //自动程序控制云台旋转
    {
        ...
        else    //未识别到目标，进入巡逻状态
        {
            ...
            {
                if (MAX_PATROL_YAW - gimbal_control_set->gimbal_yaw_motor.absolute_angle < 0.1f)
            ...
            {
                if (gimbal_control_set->gimbal_yaw_motor.absolute_angle - MIN_PATROL_YAW < 0.1f)     
            ...
            //pitch轴巡逻
            if(gimbal_control_set->pitch_patrol_dir == CCW)  //pitch轴逆时针旋转
            {
                if (MAX_PATROL_PITCH - gimbal_control_set->gimbal_pitch_motor.absolute_angle < 0.1f)
            ...
            {
                if (gimbal_control_set->gimbal_pitch_motor.absolute_angle - MIN_PATROL_PITCH< 0.1f)     
